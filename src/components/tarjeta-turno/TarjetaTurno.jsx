...
export default function TarjetaTurno({
  ...
  function getDistancia(horaInicio, horaFin) {
    ...
    return (
      <div
        className={ verifyInscripto(turno._id).some(val => val) || ((cupos.filter(cupo => cupo._id === turno._id )[0]?.totalQuantity ?? 0) === turno.max_cupos) ? "item-turno w-100 inscripto" : "item-turno w-100" }
        style={{
      ...
      >
        ...
        : (
          ((cupos.filter(cupo => cupo._id === turno._id )[0]?.totalQuantity ?? 0) === turno.max_cupos) ?
        ...
        <div className="d-flex justify-content-between">
          ...
          <span className="cupos">{cupos.filter(cupo => cupo._id === turno._id )[0]?.totalQuantity ?? 0}/{turno.max_cupos}</span>
        </div>
